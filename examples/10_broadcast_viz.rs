...
fn main() {
    ...
    println!(
        "Row Replication (Vertical Stripes) CUDA: {}",
        viz::cuda::to_cuda(&addr1.0[0].clone().simplify(), &["h", "w"])
    );
    ...
    println!(
        "Col Replication (Horizontal Stripes) CUDA: {}",
        viz::cuda::to_cuda(&addr2.0[0].clone().simplify(), &["h", "w"])
    );
    ...
    // CUDA Codegen
    let addr3 =
        scalar_broadcast.lower(&valuation, vec![ScalarExpr::Input(0), ScalarExpr::Input(1)]);
    println!(
        "Scalar Expansion CUDA: {}",
        viz::cuda::to_cuda(&addr3.0[0].clone().simplify(), &["h", "w"])
    );
