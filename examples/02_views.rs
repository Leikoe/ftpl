...
fn main() {
    ...
    println!("Transpose Target Space: {:?}", transpose.target().volume_extent());

    // 2. Reshape: Flatten (H, W) into (H*W)
    let flattened_space = Space::new(vec![
        Factor::new(Kind::Logical, Extent::Constant(32), Some("HW".to_string())),
    ]);
    let reshape = Expression::Reshape(s.clone(), flattened_space.clone());
    println!("Reshape Target Space: {:?}", reshape.target().volume_extent());
    ...
    println!("Coordinate (1, 2) in [4, 8] maps to {:?} in [32]", output_coord);
}
