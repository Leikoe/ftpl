...
fn main() {
    ...
    let hardware_primitive = Layout::row_major(frag_16x16.clone());
    ...
    let program_layout = hardware_primitive.clone().product(Layout::identity(tile_2x2));

    // 3. Verification: Can we fit the 16x16 primitive onto the 32x32 layout?
    // Using high-level left_div (via the underlying expression)
    if let Some(remainder) = program_layout.expr.left_div(hardware_primitive.expr) {
