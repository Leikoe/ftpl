...
fn main() {
    ...
    if let Some(shuffle) = l1.shuffle_to(&l_col) {
        ...
        println!("Shuffle Address Mapping for L1_offset[1] -> L_col_offset: {:?}", target_offset);
    } else {
    ...
    for (idx, stride) in conflicts {
        ...
        if stride % num_banks == 0 && stride != 0 {
            println!("  [!] SEVERE BANK CONFLICT DETECTED: Stride {} is a multiple of {} banks.", stride, num_banks);
        } else if stride > 1 {
            println!("  [!] Potential uncoalesced access detected (Stride {}).", stride);
        } else {
